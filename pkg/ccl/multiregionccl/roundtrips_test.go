...
func TestEnsureLocalReadsOnGlobalTables(t *testing.T) {
	...
	numServers := 3
	tc, sqlDB, cleanup := multiregionccltestutils.TestingCreateMultiRegionCluster(t, numServers, knobs, nil, base.ReplicationManual)
	defer cleanup()
	...
	for i := 0; i < numServers; i++ {
		...
		log.Infof(context.Background(), "!!!!! %v", rec)
