...
func TestEnsureLocalReadsOnGlobalTables(t *testing.T) {
	...
	var tableID uint32
	err = sqlDB.QueryRow(`SELECT id from system.namespace WHERE name='test'`).Scan(&tableID)
	require.NoError(t, err)

	tablePrefix := keys.MustAddr(keys.SystemSQLCodec.TablePrefix(tableID))

	tableRangeDesc := tc.LookupRangeOrFatal(t, roachpb.Key(tablePrefix))
	leaseholderStore, err := tc.Server(0).GetStores().(*kvserver.Stores).GetStore(
		tc.Server(0).GetFirstStoreID(),
	)
	require.NoError(t, err)
	require.NotNil(t, leaseholderStore)
	leaseholderRepl, err := leaseholderStore.GetReplica(tableRangeDesc.RangeID)
	for _, queueName := range []string{"replicate", "raftsnapshot"} {
		recording, pErr, err := leaseholderStore.ManuallyEnqueue(
			context.Background(), queueName, leaseholderRepl, true, /* skipShouldQueue */
		)
		require.NoError(t, pErr)
		require.NoError(t, err)
		log.Infof(context.Background(), "!!!!!!!! %s", recording)
	}
