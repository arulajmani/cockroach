...
func MakeNameMetadataKey(
	...
	if settings != nil && !settings.Version.IsActive(cluster.VersionNamespaceTableUngossip) {
		MakeDeprecatedNameMetadataKey(parentID, name)
	}
...
// This returns the key for a name, as expected by versions < 20.1 . Pass
// name == "" in order to generate the prefix key to use to scan over all of the
// names for the specified parentID.
